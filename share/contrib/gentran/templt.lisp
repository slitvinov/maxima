...
(defun procfortcomm ()
  ...
  ; <col 1>* ... <cr> ;
  (prog (c)
	(gentran-while (member (setq c (read-char nil nil '$eof$)) '(c c *))
	       (progn (pprin2 c)
...
(defun procratcomm ()
  ...
  (prog (c)
	(pprin2 '|#|)
	(gentran-while (not (eq (setq c (read-char nil nil '$eof$)) *cr*))
	       (pprin2 c))
	(pprin2 *cr*)
...
(defun procccomm ()
  ...
  (prog (c)
	...
	(cond ((eq c '*)
	       (progn (pprin2 c)
		      (setq c (read-char nil nil '$eof$))
		      (repeat (progn (gentran-while (not (eq c '*))
					    (progn (pprin2 c)
						   (setq c (read-char nil nil '$eof$))))
				     (pprin2 c)
				     (setq c (read-char nil nil '$eof$)))
